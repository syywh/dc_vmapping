...
namespace vill {
    ...
    double ORBmatcher::SearchParralax(Frame& CurrentFrame, KeyFrame* pLastKF)
    {
	vector<uchar> status;
	vector<float> err;
	...
	image1 = pLastKF->mvImagePyramid[0];
	image2 = CurrentFrame.mImGray;
	std::vector<Point2f> goodFeatures, trackedFeatures;
	std::vector<cv::KeyPoint> vKpts = pLastKF->mvKeys;
	KeyPoint::convert(vKpts, goodFeatures);
	...
	double valParralax, numParralax;
	for(int i = 0; i < status.size(); i++ )
	{
	      ...
	      Point2f p1, p2;
	      p1 = goodFeatures[i];
	      p2 = trackedFeatures[i];
	      valParralax += sqrt(pow(p1.x - p2.x, 2)+pow(p1.y - p2.y, 2));
	      numParralax++;
	}
	cout << "Average parralax: " << valParralax/numParralax << endl;
	return valParralax/numParralax;
    }
	...
	/*构建  vpMapPointMatches[bestIdxF] = pMP */
	...
	/*根据 LastFrame.mvpMapPoints[i]& !LastFrame.mvbOutlier[i]和投影关系，
	构建CurrentFrame.mvpMapPoints[bestIdx2] = pMP; */
    int ORBmatcher::SearchByProjection(int &tried, Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono,
                                       bool checkLevel) {
        int nmatches = 0;
...
// http://graphics.stanford.edu/~seander/bithacks.html#CountBitsSetParallel
    ...
    bool ORBmatcher::FindDirectProjection(
        ...
        bool success = vill::Align2D(curr->mvImagePyramid[search_level], _patch_with_border, _patch, 10, px_scaled);
