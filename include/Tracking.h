...
// void SetLaserMap(LaserMap *pLaserMap);
...
bool printParams(std::ofstream &ofile);

// Whether use IMU
bool mbUseDirect = true; //svo, false为orb slam
bool mbUseIMU = false;
bool mbUseLaserpose = false;
...
// // #ifdef TEST_STEREO
// SE3f GrabImageStereoVI(const cv::Mat &imLeft, const cv::Mat &imRight, const std::vector<IMUData> &vimu, const double &timestamp);
// // #endif

// SE3f GrabImageStereoVIlaser(const cv::Mat &imLeft, const cv::Mat &imRight, 
// 					   const std::vector<IMUData> &vimu, const double &timestamp,
// 					   const Matrix4f &laserpose, const double &timestamplaserpose);
...
int PoseOpt(Frame *pFrame);
// int PoseOpt(Frame *pFrame, Frame *pLastFrame, const IMUPreintegrator &imupreint, const Vector3d &gw,
// 	const bool &bComputeMarg = false);
// int PoseOpt(Frame *pFrame, KeyFrame *pLastKF, const IMUPreintegrator &imupreint, const Vector3d &gw,
// 	const bool &bComputeMarg = false);

bool getRelocalizationState();
void setRelocalizationState(bool flag);

bool getPubLoc();
void setPubLoc(bool flag);

void EqualizedImageLog(cv::Mat& oriImage, cv::Mat& dstImage);

protected:

// Main tracking function. It is independent of the input sensor.
void Track();
bool routineOnlyTracking(bool bMapUpdated);
void CopyPose(void);
bool CheckMapUpdated(void);
...
// 注意会设置当前帧与地图点的关联
bool TrackWithFeature(bool bTrackLastKF);
...
bool Relocalization();
bool Relocalization_PnP();
bool Relocalization_2P();
...
// added by hzliuhaiwei
int minInitFeaturePoints;
int minInitMapPoints;
int minInitTrackPoints;  
int minTrackPoints;
int minLocalMapTrackPoints;
int mverbose;
ofstream frouteTrack;
ofstream frtPose;

double lastRelocTime;
std::mutex mMutexReLocalizationUpdate, mMutexPubLoc;
bool mbRelocalizationSuccess;
bool mbPubLoc;
